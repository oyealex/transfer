...
通过 `welink-cli` 查询 WeLink 群聊或私聊消息，自动分页，支持丰富的过滤条件。封装脚本为本 SKILL.md 同目录下的 `welink_msg_filter.py`，Agent 只需调用它并解析 JSON 输出即可。

> **路径约定**：本文中所有 `./` 均相对于本 SKILL.md 所在目录。
