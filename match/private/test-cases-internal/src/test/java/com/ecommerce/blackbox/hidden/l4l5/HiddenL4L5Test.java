...
class HiddenL4L5Test extends BlackboxTestBase {
    ...
    // ----------------------------------------------------------------
    // L4-07: Unified notification recording (requires /api/v1/admin/notifications)
    // ----------------------------------------------------------------

    @Test
    @DisplayName("HID-L4-07: unified notification record exists with bizType/templateCode/channel/idempotencyKey")
    void l4_07_unifiedNotificationRecording() {
        // --- Given F-USER (trigger notification via user registration or order) ---
        UserFixture userFixture = new UserFixture(apiClient, testRunContext);
        ActivatedUser user = userFixture.registerAndActivateUser(testRunContext);
        AddressResult addr = userFixture.createAddress(testRunContext, user.getToken(),
                "Guangdong", "Shenzhen", "Nanshan", "No.1 Tech Street");

        // --- Given create order + payment to trigger notifications ---
        ProductFixture productFixture = new ProductFixture(apiClient, testRunContext);
        InventoryFixture inventoryFixture = new InventoryFixture(apiClient, testRunContext);
        SkuResult sku = productFixture.createOnShelfSku(testRunContext, testRunContext.getAdminToken(),
                testRunContext.uniqueSkuCode(), new BigDecimal("50.00"));
        ...
        notifyHeaders.setBearerAuth(testRunContext.getAdminToken());
        ResponseEntity<String> notifyResp = apiClient.get(
                "/api/v1/admin/notifications?bizId=" + order.getOrderId(), notifyHeaders);

        // --- Then ---
        assertTrue(notifyResp.getStatusCode().is2xxSuccessful(),
                "/api/v1/admin/notifications should return 200");

        String notifyBody = notifyResp.getBody();
        assertNotNull(notifyBody, "notification response should not be null");

        // --- Then notification record contains bizType, templateCode, channel,
        // idempotencyKey ---
        // These fields are part of the unified notification contract
        boolean hasBizType = notifyBody.contains("bizType") || notifyBody.contains("biz_type");
        boolean hasTemplateCode = notifyBody.contains("templateCode") || notifyBody.contains("template_code");
        boolean hasChannel = notifyBody.contains("channel");
        boolean hasIdempotencyKey = notifyBody.contains("idempotencyKey")
                || notifyBody.contains("idempotency_key");

        assertTrue(hasBizType || hasTemplateCode || hasChannel || hasIdempotencyKey,
                "notification record should contain at least one of: bizType, templateCode, channel, idempotencyKey");

        // --- Then: does NOT depend on log or Mock sender internal state ---
        // (proven by querying via REST endpoint rather than reading logs)
    }
