...
class HiddenL2Test extends BlackboxTestBase {
    ...
    @Test
    @DisplayName("HID-L2-02: paying 1.00 for 100.00 order returns PAYMENT_AMOUNT_MISMATCH")
    void l2_02_partialPaymentRejected() {
        // --- Given F-ORDER payableAmount=100.00 ---
        UserFixture userFixture = new UserFixture(apiClient, testRunContext);
        ActivatedUser user = userFixture.registerAndActivateUser(testRunContext);
        AddressResult addr = userFixture.createAddress(testRunContext, user.getToken(),
                "Guangdong", "Shenzhen", "Nanshan", "No.1 Tech Street");
    ...
    @Test
    @DisplayName("HID-L2-06: refundAmount=paidAmount*0.98 (98.00 for 100.00 paid)")
    void l2_06_refundAmountCalculation() {
        ...
        if (refundAmountStr != null) {
            assertEquals(0, new BigDecimal("98.00").compareTo(new BigDecimal(refundAmountStr)),
                    "refundAmount should be 98.00 (100 * 0.98)");
        }
    }
    ...
    // ------------------------------------------------------------------
    // HID-L2-08: Loyalty activity multiplier
    // ------------------------------------------------------------------

    @Test
    @DisplayName("HID-L2-08: loyalty activity-multiplier=2.0 → 200 points for 100.00 paid")
    void l2_08_loyaltyActivityMultiplier() {
        // --- Given PUT /api/v1/admin/system/configs/loyalty.activity-multiplier set loyalty.activity-multiplier=2.0 ---
        Map<String, Object> configBody = new LinkedHashMap<>();
        configBody.put("value", "2.0");
        ...
        ResponseEntity<String> configResp = apiClient.put("/api/v1/admin/system/configs/loyalty.activity-multiplier", configBody, configHeaders);

        if (configResp.getStatusCode().value() == HttpStatus.NOT_FOUND.value()) {
            // config not available — note as not supported
            return;
        }

        // --- Given F-PAID-ORDER paidAmount=100.00 with multiplier=1.0 ---
        UserFixture userFixture = new UserFixture(apiClient, testRunContext);
        ActivatedUser user = userFixture.registerAndActivateUser(testRunContext);
        AddressResult addr = userFixture.createAddress(testRunContext, user.getToken(),
                "Guangdong", "Shenzhen", "Nanshan", "No.1 Tech Street");
        ...
        // --- Then this order earned 200 points ---
        if (historyResp.getStatusCode().is2xxSuccessful()) {
            String body = historyResp.getBody();
            assertNotNull(body, "points history should be returned");
            assertTrue(body.contains("200") || body.contains("\"points\":200"),
                    "with activity-multiplier=2.0, 100.00 order should earn 200 points");
        }
    }
    ...
    void l2_09_partialInvoiceSupport() {
        ...
        // --- Then both invoices created, cumulative amount=500.00 ---
    ...
    void l2_12_sensitiveWordContainsMatch() {
        // --- Given F-PAID-ORDER to create a reviewable context ---
        UserFixture userFixture = new UserFixture(apiClient, testRunContext);
        ActivatedUser user = userFixture.registerAndActivateUser(testRunContext);
        AddressResult addr = userFixture.createAddress(testRunContext, user.getToken(),
                "Guangdong", "Shenzhen", "Nanshan", "No.1 Tech Street");
