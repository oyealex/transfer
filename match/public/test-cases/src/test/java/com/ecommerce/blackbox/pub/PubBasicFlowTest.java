...
class PubBasicFlowTest extends BlackboxTestBase {
    ...
    void pub001_registerActivateLogin() {
        ...
        // --- Then 1: PENDING_ACTIVATION and activation token generated ---
        assertNotNull(regResult.getUserId());
        assertEquals(testRunContext.uniqueEmail(), regResult.getEmail());
        assertEquals("PENDING_ACTIVATION", regResult.getStatus());
        assertNotNull(regResult.getActivationToken());

        // --- When 2: Activate ---
        Map<String, Object> actBody = new LinkedHashMap<>();
        actBody.put("token", regResult.getActivationToken());
        ...
        assertEquals(regResult.getEmail(), apiClient.readJsonPath(meResp, "$.email"));
    ...
    @Test
    @DisplayName("PUB-009: Create a payment for a created order and verify payment status is CREATED")
    void pub009_createPayment() {
        ...
        // Then 3: status=CREATED
        assertEquals("CREATED", paymentResult.getStatus());
    }
    ...
    void pub014_createReview() {
        ...
        if (logistics.getShipmentId() != null && !logistics.getShipmentId().isEmpty()) {
            ...
            LogisticsResult afterOutbound = logisticsFixture.getLogistics(
                    testRunContext, user.getToken(), orderResult.getOrderId());
            if (afterOutbound.getTrackingNo() != null && !afterOutbound.getTrackingNo().isEmpty()) {
                logisticsFixture.logisticsCallback(testRunContext,
                        afterOutbound.getTrackingNo(), "DELIVERED");
            }
        }
        ...
        ReviewFixture.ReviewResult reviewResult = reviewFixture.createReview(
                ...
                orderResult.getOrderId(), orderItemId, 5, reviewContent);
    ...
    void pub015_salesStatistics() {
        ...
        // Then 2: totalOrders includes the order paid in this test
        ...
        assertTrue(totalOrders >= 1, "totalOrders should include the paid order");
