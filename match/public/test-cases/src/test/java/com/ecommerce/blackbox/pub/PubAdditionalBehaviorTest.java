 ...
 */
@DisplayName("Additional Behavior Tests")
class PubAdditionalBehaviorTest extends BlackboxTestBase {

    // ----------------------------------------------------------------
    // PUB-101: 80%-price coupon discount calculation
    // ----------------------------------------------------------------

    @Test
    @DisplayName("PUB-101: An 80%-price coupon should yield a discount of 20% of the item price")
    void pub101_couponDiscountShouldBeCorrect() {
        ...
        assertEquals(0, new BigDecimal("20.00").compareTo(orderResult.getDiscountAmount()),
                "discountAmount should be 20.00 for an 80%-price coupon on a 100.00 item");
    ...
    void pub108_paymentSuccessShouldNotBeBlockedByPostActions() {
        ...
        try {
            ...
            if (orderPayStatus == null) {
                orderPayStatus = apiClient.readJsonPath(orderResp, "$.status");
            }
            if (orderPayStatus == null) {
                orderPayStatus = apiClient.readJsonPath(orderResp, "$.data.status");
            }
