...
public class UserFixture extends BlackboxTestBase {
     ...
     * <p>
     * If the current implementation sets users to ACTIVE immediately during
     * registration, this call may fail. Callers should use
     * {@link #registerAndActivateUser(TestRunContext)} for the standard flow
     ...
     * <p>
     * Handles the case where users are marked ACTIVE directly during registration
     * (no activation token generated).
     *
     * @param ctx per-test unique business-key context
    ...
    public ActivatedUser registerAndActivateUser(TestRunContext ctx) {
        ...
        // If an activation token is present, attempt activation (silently tolerate
        // failure when user is already ACTIVE).
