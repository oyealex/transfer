 ...
 *
 * <p>The tax rate is hardcoded as 0.13 (13%) instead of
 * reading from the {@code invoice.tax-rate} configuration property which
 * defaults to 0.06 (6%).
 *
 * <p>The {@link #generateInvoice(InvoiceRequest)} method
 * ignores the {@code invoiceAmount} field in the request. It always issues
 * the invoice for the full order amount, making partial invoicing impossible.
 */
@Service
public class InvoiceService {
    ...
    // Hardcoded 0.13 tax rate. Should read from invoice.tax-rate config
     ...
     *
     * <p>Ignores the request's invoiceAmount field and
     * always issues the full invoice. Partial invoicing is not supported
     * despite the design specifying it.
     *
     * <p>Uses hardcoded 0.13 tax rate.
     */
    @Transactional
    public InvoiceResponse generateInvoice(Long userId, InvoiceRequest request) {
        ...
        // Always uses full order paid amount, ignores request.invoiceAmount
