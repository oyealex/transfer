...
/**
 * Unit tests for {@link SensitiveWordFilter} verifying ACTUAL behavior.
 *
 * <p>Uses exact match (equals) instead of contains.
 * A review containing "badword here" passes the filter because the content
 * string is not exactly equal to "badword".
 */
...
class SensitiveWordFilterTest {
    ...
    void testFilter_containsBadWord_passes() {
        ...
        // equals() is used instead of contains(), so this passes through
    ...
    void testFilter_containsBadWord_notReplaced() {
        ...
        // equals() used instead of contains(), so no replacement occurs
    ...
    void testFilter_nullContent_handled() {
        ...
        // filter iterates over words and calls sw.getWord().equals(result)
        // where result is the content parameter. If content is null, this
        // will NPE on equals(). This test documents the current behavior.
        try {
            String result = filter.filter(null);
            // If no NPE, record the result
            assertThat(result).isNull();
        } catch (NullPointerException e) {
            // Current behavior: NPE on null content
            // This is acceptable — the controller validates @NotBlank
        }
    }
}
