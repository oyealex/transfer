 ...
 *
 * <p>{@link #calculateDiscount(BigDecimal, CouponTemplate)}
 * for DISCOUNT type coupons multiplies price by (1 - discountValue) instead of
 * discountValue. When discountValue=0.8 (80% off), the result is price*0.2
 * instead of price*0.8, causing a massive undercharge.
 */
@Service
public class CouponService {
     ...
     *
     * <p>For DISCOUNT type, the formula is:
     * <pre>price * (1 - discountValue)</pre>
     * When discountValue=0.8 (representing 80% off / keep 80%), this computes
     * price*0.2 instead of the expected price*0.8. The resulting order total
     * is drastically lower than intended.
     */
    public BigDecimal calculateDiscount(BigDecimal price, CouponTemplate coupon) {
        ...
        switch (coupon.getType()) {
            case DISCOUNT:
                // Uses price * (1 - discountValue) instead of price * discountValue
                // e.g. 0.8 coupon → price*(1-0.8) = price*0.2 instead of price*0.8
