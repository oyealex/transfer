...
import com.ecommerce.blackbox.common.BlackboxHarnessConfig;
...
public class UserFixture extends BlackboxTestBase {
     ...
     * @return parsed {@link RegisterResult}; activationToken may be supplied by
     * the test harness when the REST response follows the public API contract.
     */
    ...
    private RegisterResult parseRegisterResult(String json) {
        try {
            ...
            if (data.has("userId")) {
                result.setUserId(data.get("userId").asLong());
            }
            if (data.has("email")) {
                result.setEmail(data.get("email").asText());
            }
            if (data.has("status")) {
                result.setStatus(data.get("status").asText());
            }
            if (data.has("activationToken")) {
                result.setActivationToken(data.get("activationToken").asText());
            }
            if (result.getActivationToken() == null) {
                result.setActivationToken(
                        BlackboxHarnessConfig.findActivationTokenForUser(result.getUserId()));
            }
            return result;
        } catch (Exception e) {
