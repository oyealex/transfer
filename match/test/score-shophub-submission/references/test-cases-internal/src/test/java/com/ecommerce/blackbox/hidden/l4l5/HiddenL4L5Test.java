...
class HiddenL4L5Test extends BlackboxTestBase {
    ...
    void l4_01_paymentOrderQueryBoundary() {
        ...
        BigDecimal retryPayableAmount = order2.getPayableAmount() != null
                && order2.getPayableAmount().compareTo(BigDecimal.ZERO) > 0
                ? order2.getPayableAmount() : new BigDecimal("50.00");
        PaymentResult payment2 = paymentFixture.pay(testRunContext, user.getToken(),
                order2.getOrderId(), retryPayableAmount,
    ...
    @Test
    @DisplayName("HID-L5-02: loyalty listener failure is recorded and remains retryable when metadata is exposed")
    void l5_02_eventFailureRecordGenerated() {
        ...
        // --- Then: if retryability metadata is exposed, it should indicate the
        // event can be retried. The design does not freeze failure-record field
        // names beyond requiring the failure record to be queryable.
        boolean hasRetryable = failuresBody.contains("retryable");
