...
class HiddenE2ETest extends BlackboxTestBase {
    ...
    @Test
    @DisplayName("HID-E2E-06: paid → refund apply → admin review → warehouse accept → refundAmount=paidAmount*0.98")
    void e2e06_refundReviewAcceptFlow() {
        ...
        // --- Then: refund amount reflects 2% fee on actual paid amount ---
        ResponseEntity<String> refundResp = apiClient.get(
                "/api/v1/refunds/" + refundApply.getRefundId(), userHeaders(user.getToken()));
        ...
        if (refundAmountStr != null) {
            ...
            assertEquals(0, expectedRefundAmount.compareTo(actualRefundAmount),
                    "refund amount should be paidAmount * 0.98");
        }
    ...
    @Test
    @DisplayName("HID-E2E-07: review approval awards points when review is accepted")
    ...
    void e2e08_invoiceSettlementFlow() {
        ...
        // --- When POST /api/v1/invoices (remaining paid amount) ---
        ...
        InvoiceResult invoice3 = invoiceFixture.createInvoice(testRunContext, user.getToken(),
                ...
                "Over-limit Invoice");
        assertNull(invoice3.getInvoiceId(), "invoice exceeding paid amount should be rejected");
        if (invoice3.getInvoiceId() == null) {
            // This is expected -- the invoice should be rejected
            // We can verify via the raw response
            Map<String, Object> overloadBody = new LinkedHashMap<>();
            overloadBody.put("orderId", order.getOrderId());
            overloadBody.put("invoiceType", "PERSONAL");
            overloadBody.put("invoiceAmount", new BigDecimal("0.01"));
            overloadBody.put("invoiceTitle", "Over-limit Invoice");
            overloadBody.put("invoiceRequestNo",
                    "IR-OVER-" + testRunContext.getTestRunId() + "-" + order.getOrderId());
