...
public class OrderCancelService {
    ...
    /**
     * Cancel a PAID order: enters CANCEL_REVIEWING for admin review.
     * The order is NOT directly cancelled. Admin must approve via reviewCancel().
     */
    private CancelOrderResponse cancelPaidOrderDirectly(Order order, String reason) {
        ...
        orderService.recordEvent(order.getId(), fromStatus, OrderStatus.CANCEL_REVIEWING,
                ...
                "User requested cancellation of paid order: " + reason);

        log.info("PAID order {} moved to CANCEL_REVIEWING for admin review",
                order.getId());

        return new CancelOrderResponse(order.getId(), OrderStatus.CANCEL_REVIEWING.name(),
                "Cancellation request submitted for admin review. Refund will be processed after approval.");
    }
