...
public class DomainEventPublisher {
    ...
    private void persistFailure(AbstractDomainEvent event, Exception exception) {
        try {
            ...
            record.setEventType(event.getClass().getName());
            record.setEventId(event.getEventId());
            record.setListenerName("ApplicationEventPublisher");
    ...
    /**
     * Replays all non-retried failed events by deserializing and re-publishing them.
     *
     * @return the number of events successfully replayed
     */
    public int replayAll() {
        var failedEvents = failedEventRecordRepository.findAll().stream()
                .filter(r -> !r.isRetried())
                .toList();

        int replayed = 0;
        for (FailedEventRecord record : failedEvents) {
            try {
                Class<?> eventClass = Class.forName(record.getEventType());
                AbstractDomainEvent event = (AbstractDomainEvent) objectMapper.readValue(
                        record.getEventPayload(), eventClass);
                applicationEventPublisher.publishEvent(event);
                record.setRetried(true);
                record.setRetryCount(record.getRetryCount() + 1);
                failedEventRecordRepository.save(record);
                replayed++;
                log.info("Replayed failed event: id={}, type={}", record.getId(), record.getEventType());
            } catch (Exception e) {
                log.error("Failed to replay event record id={}, type={}: {}",
                        record.getId(), record.getEventType(), e.getMessage());
                record.setRetryCount(record.getRetryCount() + 1);
                failedEventRecordRepository.save(record);
            }
        }
