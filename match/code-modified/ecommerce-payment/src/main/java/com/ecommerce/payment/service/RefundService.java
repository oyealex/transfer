...
public class RefundService {
    ...
    public RefundResponse applyRefund(Long userId, RefundApplyRequest request) {
        ...
        BigDecimal refundBase = payment.getPaidAmount();
        try {
            OrderDto order = orderQueryService.getOrder(request.getOrderId());
            if (order.getItemTotal() != null && order.getItemTotal().compareTo(BigDecimal.ZERO) > 0) {
                refundBase = order.getItemTotal();
            }
        } catch (Exception e) {
            log.warn("Falling back to payment amount for refund calculation, orderId={}, reason={}",
                    request.getOrderId(), e.getMessage());
        }
        BigDecimal refundAmount = refundCalculator.calculate(refundBase);
    ...
    /**
     * Approves a refund — moves to WAITING_WAREHOUSE_ACCEPT.
     * The refund is NOT completed until warehouse accepts the returned goods.
     */
    private void approveRefund(Long refundId, Long reviewerId, String note) {
        RefundRecord refund = refundRecordRepository.findById(refundId)
                .orElseThrow(() -> new ResourceNotFoundException("RefundRecord", refundId));

        // L3-02: Go to WAITING_WAREHOUSE_ACCEPT instead of directly completing
        refund.setStatus(RefundStatus.WAITING_WAREHOUSE_ACCEPT);
        refund.setReviewerId(reviewerId);
        refund.setReviewNote(note);
        refundRecordRepository.save(refund);

        log.info("Refund {} approved, awaiting warehouse acceptance", refund.getRefundNo());
    }
    ...
    public RefundResponse warehouseAccept(Long refundId, Long acceptorId) {
        ...
        log.info("Warehouse accepted refund: refundNo={}, status={}", refund.getRefundNo(), refund.getStatus());
        return toRefundResponse(refund);
    }
