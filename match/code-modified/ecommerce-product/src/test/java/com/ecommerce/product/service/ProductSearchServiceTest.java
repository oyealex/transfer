...
class ProductSearchServiceTest {
    ...
    @Test
    @DisplayName("search with onlyOnShelf=false returns all non-DELETED SKUs including OFF_SHELF and DRAFT")
    void testSearch_withOnlyOnShelfFalse_returnsAllNonDeletedSkus() {
        // Explicitly false: excludes only DELETED, includes OFF_SHELF and DRAFT.
        ...
        // OFF_SHELF and DRAFT included in results
    ...
    @Test
    @DisplayName("search with default onlyOnShelf (true) returns only ON_SHELF products")
    void testSearch_defaultOnlyOnShelf_returnsOnlyOnShelf() {
        // Default: onlyOnShelf is true (not set), filters to ON_SHELF only.
        ...
        ProductSearchRequest request = new ProductSearchRequest();
        // onlyOnShelf defaults to true — we do NOT set it
        PageResponse<ProductListResponse> result = productSearchService.search(request);

        // ON_SHELF is included; OFF_SHELF and DRAFT are filtered at Specification level
