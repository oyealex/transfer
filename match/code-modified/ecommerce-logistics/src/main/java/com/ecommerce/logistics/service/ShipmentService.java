...
public class ShipmentService {
    ...
    public void pick(Long shipmentId, Long pickerId) {
        ...
        recordTracking(shipmentId, "PICKING", "Warehouse",
                "Picking started by operator " + (pickerId != null ? pickerId : "SYSTEM"),
    ...
    public void outbound(Long shipmentId) {
        ...
        // L3-04: Must have completed picking and label printing before outbound
        if (shipment.getStatus() != ShipmentStatus.LABEL_PRINTED) {
            throw new IllegalStateException(
                    "Cannot outbound shipment in status " + shipment.getStatus()
                            + ". Must be LABEL_PRINTED.");
        }
